...
def init_tables(stl_api_key):
    ...
    if SERVICE_DEFINITIONS_TABLE_NAME not in existing_tables:
        ...
        service_codes = get_child_service_codes(get_requests_from_stl(STL_API_URL, SERVICES_TABLE_NAME, stl_api_key))

        for service_code in service_codes:
            insert_resources_with_id(db_resource_table(SERVICE_DEFINITIONS_TABLE_NAME),
                                     get_resource_response_with_id(STL_API_URL,
                                                                   SERVICES_RESOURCE,
                                                                   service_code,
                                                                   stl_api_key))
